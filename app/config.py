...
class Config:
    ...
    # persistent storage (a Railway Volume, or a Render Disk mounted at
    # this path). For a real production launch, prefer a persistent object
    # store such as Cloudinary or AWS S3 over local disk regardless.
    ...
    # every deploy; Render sets RENDER=true the same way. FLASK_ENV is kept
    # as a manual override for local testing against a production-like
    # config. Any one of these flips on secure cookies.
    SESSION_COOKIE_SECURE = bool(os.environ.get("RAILWAY_ENVIRONMENT")) or bool(os.environ.get("RENDER")) or os.environ.get("FLASK_ENV") == "production"
